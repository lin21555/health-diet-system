...
## 本次更新：AI问答回到QQ聊天样式，并补充右侧滚动条和底部输入框

- 右侧聊天区域新增独立滚动条，不再跟随整个页面滚动；
- 右侧聊天面板底部新增输入框、发送按钮和快捷操作；
...
前台将原“食谱中心”拆分为：
- 食材查询：展示常见食材名称、图片、热量、蛋白质，支持分类、搜索和详情页；
- 食谱查询：展示常见菜谱名称、图片、热量、蛋白质，支持分类、搜索和详情页。

后台同步增加：
- 食材管理：食材增删改查、详情预览；
- 食谱管理：原菜品管理更名为食谱管理。

如果你的本地数据库已经导入过旧版本，请执行：
`database/update_food_query_module.sql`


## 本次更新：健康活动模块

前台新增：
- 健康活动列表：展示活动标题、图片、地点、时间、报名截止、状态、报名人数；
- 活动详情页：展示完整活动信息；
- 进行中的活动支持报名参加；
- 个人中心新增“我的活动”标签页，展示已报名活动。

后台新增：
- 健康活动管理：新增、编辑、删除、查看活动；
- 数据统计新增健康活动数量。

数据库需执行：
`database/update_health_activity_module.sql`
